...
class RandomShift(BaseOperator):
    """
    Randomly shift image without cahnging the image frame.
    WARNING: does not apply to any other annotations other than bboxes.
    Args:
    ...
    # what is the coordinate when img is shifted by (h_shift, w_shift)?
    # return xyxy
    def calc_shift_coor(self, im_h, im_w, shift_h, shift_w):
        return [max(0, 0+shift_w), max(0, 0+shift_h),
